                           ...
                           <table id="DOM-datatable" class="table table-bordered table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                              ...
                              <tbody>
                                <?php
                                    ...
                                    $s = "SELECT Dated,Odometer,UserID FROM `user_jobs` WHERE Dated BETWEEN '$epochfirst' AND '$epochlast'";
                                    $q = mysqli_query($conn,$s);
                                    $z = "SELECT Username,UserID FROM `user_profile`";
                                    $y = mysqli_query($conn,$z);
                                    $rows = array();
                                    while($userdata = mysqli_fetch_array($y))
                                    {
                                      $totaljobs = 0;
                                      $totaldistance = 0;
                                      if(sizeof($rows) > 0) {
                                        ...
                                        {
                                          if($jobdata["UserID"] == $userdata["UserID"]) {
                                            $totaljobs++;
                                            $totaldistance += $jobdata["Odometer"];
                                            //continue;
                                          } else {
                                           // echo "UHOh";
                                           // continue;
